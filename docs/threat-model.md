...
| App-switcher snapshot / backgrounding                 | Shared lock machine with zero-grace config (`backgroundLockMs: 0`): every real backgrounding clears the JS secret cache and requires re-authentication. The snapshot itself is covered natively on `willResignActive` — an OPAQUE, secret-independent view above the key window, never a blur (a blurred six-digit code is not redacted: a million candidate renderings against a known font and layout make it classifiable) — because iOS photographs the screen before any JS listener runs. After a real backgrounding the cover is held until the JS layout acknowledges it has drawn a safe frame, so the first frame on resume cannot be the vault the user left |
| Copied code read by another app                       | Copying is native-only: `UIPasteboard` `.localOnly` (never Universal Clipboard, so the code cannot reach a paired Mac or iPad) plus an OS-enforced `.expirationDate` of 30s that survives process death. An explicit lock takes the code back if it is still ours, judged by the pasteboard change count — never by reading the pasteboard, which on iOS 16+ would raise a paste prompt at the moment the vault locks. Backgrounding deliberately does NOT wipe: switching apps is how a code gets pasted. Builds without the native module refuse to copy at all                                                                                                       |
| Hostile QR code                                       | A camera is an untrusted input channel: scanned `otpauth://` URIs are validated against the same `otpParametersSchema` the vault enforces at rest (HOTP, out-of-range periods/digits and non-Base32 secrets are refused at the scan), and the secret is encrypted and stored by the scanner itself rather than passed through a navigation parameter the router may serialize                                                                                                                                                                                                                                                                                           |
