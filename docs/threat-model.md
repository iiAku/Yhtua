...
| App-switcher snapshot / backgrounding                 | Shared lock machine with zero-grace config (`backgroundLockMs: 0`): every real backgrounding clears the JS secret cache and requires re-authentication. The snapshot itself is masked natively on `willResignActive` (a blur view above the key window), because iOS photographs the screen before any JS listener runs                                                                                                                                                                                                                                           |
